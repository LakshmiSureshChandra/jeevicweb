            ...
            <div className="flex flex-col gap-8">
              ...
              <div className="flex flex-col lg:flex-row gap-6 px-4">
                ...
                <div className="lg:hidden w-full">
                  <div className="group relative overflow-hidden rounded-2xl bg-white shadow-md hover:shadow-xl transition-all duration-300">
                    <div className="aspect-[16/9] lg:aspect-[4/5] overflow-hidden">
                      <img 
                        src={productsData.flashSaleCards[0].image} 
                        alt={productsData.flashSaleCards[0].name}
                        className="w-full h-full object-cover transform group-hover:scale-105 transition-transform duration-500"
                      />
                      <div className="absolute top-3 right-3">
                        <span className="bg-red-600 text-white px-3 py-1.5 rounded-full text-sm font-medium">
                          -{productsData.flashSaleCards[0].discount}
                        </span>
                      </div>
                    </div>
                    <div className="absolute bottom-0 inset-x-0 p-4 bg-gradient-to-t from-black/80 to-transparent">
                      <h3 className="text-white font-bold text-xl mb-1">{productsData.flashSaleCards[0].name}</h3>
                      <p className="text-white/80 text-sm mb-3 line-clamp-2">{productsData.flashSaleCards[0].description}</p>
                      <div className="flex items-center justify-between">
                        <div className="flex flex-col">
                          <span className="text-white text-xl font-bold">{productsData.flashSaleCards[0].discountedPrice}</span>
                          <span className="text-white/60 text-sm line-through">{productsData.flashSaleCards[0].originalPrice}</span>
                        </div>
                        <button className="bg-white text-black px-4 py-2 rounded-full text-sm font-medium hover:bg-black hover:text-white transition-colors duration-300">
                          Shop Now
                        </button>
                      </div>
                    </div>
                ...
                <div className="hidden lg:block w-[400px] flex-shrink-0">
                  <div className="group relative h-full overflow-hidden rounded-2xl bg-white shadow-md hover:shadow-xl transition-all duration-300">
                    <div className="aspect-[4/5] overflow-hidden">
                      ...
                      />
                      <div className="absolute top-3 right-3">
                        <span className="bg-red-600 text-white px-4 py-2 rounded-full text-base font-medium">
                          -{productsData.flashSaleCards[0].discount}
                        </span>
                      </div>
                    </div>
                    <div className="absolute bottom-0 inset-x-0 p-6 bg-gradient-to-t from-black/80 to-transparent">
                      <h3 className="text-white font-bold text-2xl mb-2">{productsData.flashSaleCards[0].name}</h3>
                      <p className="text-white/80 text-base mb-4">{productsData.flashSaleCards[0].description}</p>
                      <div className="flex items-center justify-between">
                        <div className="flex flex-col">
                          <span className="text-white text-2xl font-bold">{productsData.flashSaleCards[0].discountedPrice}</span>
                          <span className="text-white/60 text-lg line-through">{productsData.flashSaleCards[0].originalPrice}</span>
                        </div>
                        <button className="bg-white text-black px-6 py-3 rounded-full text-base font-medium hover:bg-black hover:text-white transition-colors duration-300">
                          Shop Now
                        </button>
                      </div>
                    </div>
                ...
                <div className="flex overflow-x-auto gap-6 px-4 pb-4 hide-scrollbar snap-x snap-mandatory">
                  {productsData.justForYouCards.map((product, index) => (
                    ...
                    >
                      ...
                      <div className="absolute bottom-0 left-0 right-0 p-4 text-white transform md:translate-y-full md:group-hover:translate-y-0 transition-transform duration-300">
